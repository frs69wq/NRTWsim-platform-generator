...
void load_platform(const sg4::Engine& e)
{
  ...
  // std::cout << "Platform loaded from config: " << config_path.c_str() << std::endl;

  // Create storage system zones (if any at the top level)
  ...
  // Parse the new platform cluster multiple json file and create inter dc links
  ...
  // Create routes between datacenters
  if (config.contains("routes")) {
    for (const auto& route_cfg : config["routes"]) {
      const std::string src_name  = route_cfg["src"];
      const std::string dst_name  = route_cfg["dst"];
      auto* src_dc = zone_map[src_name];
      auto* dst_dc = zone_map[dst_name];
      ...
      e.get_netzone_root()->add_route(src_dc, dst_dc, route_links);
    }
  }
